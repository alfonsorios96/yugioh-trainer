...
3. WindBot joins with `Deck=<executor>` (Blue-Eyes / SkyStriker / Tearlaments / Toon2026Agent).
4. For **Toon 2026 Agent**, install the executor once (needs git + Mono `msbuild`/`xbuild`):

```bash
npm run install:engines -- /path/to/your/EDOPro
```

That writes `WindBot/Decks/AI_Toon2026.ydk`, merges Toon 2026 Agent into `WindBot/bots.json`, and rebuilds `WindBot.exe` with the new engine (original exe saved as `WindBot.exe.ygo-trainer-bak`). If your install already has `ExecutorBase.dll`, it compiles `WindBot/Executors/YgoTrainerEngines.dll` instead.
